...
module BookFunctions
  def list_all_books
    @book_list.each_with_index do |book, i|
      puts "#{i+1}) publisher: #{book.publisher}, cover state: #{book.cover_state}, publish date: #{book.publish_date.year}"
    end
  end
  ...
  def get_cover_state
    ...
    while cover_state == ""
      ...
      valid = cover_state == 'good' || cover_state == 'bad'
  ...
  def get_publish_date
    publish_date = '0'
    until !!(publish_date =~ /\d{4,4}/) && publish_date.to_i.between?(1, 2022) 
  ...
  def add_book
    ...
    publish_date = get_publish_date().to_i
